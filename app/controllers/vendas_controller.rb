class VendasController < ApplicationController
  ...
  # GET /vendas.json
  def index
    @q = Venda.ransack(params[:q].try(:merge, m: 'or' ))
    @vendas = @q.result(distinct: true)
    #@vendas = Venda.all
  end
  ...
  private
    ...
    def venda_params
      params.require(:venda).permit(:data, :valor_bruto, :valor_venda, :lucro, :produto_id, :despesa_id, :quantidade_id, :compra_id)
    end
end
